...
- **Dhaka Now (events):** a separate `events` collection of YAML files in `src/content/events/`, shown on the homepage and at `/whats-on/` (`src/pages/whats-on.astro`). Past events are dropped at build time and again in the browser, comparing dates in Dhaka time, so the list stays correct between deploys. Event dates follow the same fact-checking rule: only add dates confirmed by the organiser or an official source.
